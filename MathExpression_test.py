...
class TestMathExpression(unittest.TestCase):
    def test_MathExpression(self):
        ...
        # test6
        str = "a + 2 - f(b,c)*(b - c)"
        test6 = expression(str=str, func_dic=lambda a, b: a + b, a=2, b=1, c=3, d=4, e=8)
        ...
        # test8
        str = "a + 2 - f(b,c)*(b - c)"
        test6 = expression(str=str, func_dic=lambda a, b: a + b, a=2, b=1e+15, c=float('inf'), d=4, e=8)
        value6 = test6.process()
        out6 = direct_cal(value6)
        self.assertEqual(2 + 2 - (1e+15 + float('inf')) * (1e+15 - float('inf')), out6)

    @settings(max_examples=10)
    @given(a=st.floats(min_value=min, max_value=max), b=st.floats(min_value=min, max_value=max),
           c=st.floats(min_value=min, max_value=max))
    def test_PBT1(self, a, b, c):
        str = "a + 2 - f(a,b)*(b - c)"
        test = expression(str=str, func_dic=lambda a, b: a + b, a=a, b=b, c=c)
        value = test.process()
    ...
    def test_PBT3(self, a, b, c):
        str = "a + 2 - f(b, c)*(b - c)"
        func_dic = lambda a, b: sin(a) + b
        test = expression(str=str, func_dic=func_dic, a=a, b=b, c=c)
        value = test.process()
        out = direct_cal(value)
